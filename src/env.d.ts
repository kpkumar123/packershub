...
// PUBLIC_-prefixed vars are build-time, exposed via import.meta.env — a
// different mechanism from the Cloudflare.Env runtime secrets above.
// This was previously untyped (the GA4_ID entry in Cloudflare.Env below
// was dead code — Analytics.astro has only ever read
// import.meta.env.PUBLIC_GA_MEASUREMENT_ID, never GA4_ID).
...
declare global {
  namespace Cloudflare {
    interface Env {
      ...
      SITE_URL?: string;
      // v10.7.6 — these 5 were already read via env?.X in live API routes
      // (payment/create-order.ts, payment/verify.ts, voice/*.ts,
      // admin/index-ping.ts, gps/update.ts) but were missing from this
      // interface, so TypeScript gave no warning that they were unset.
      // Razorpay checkout (src/pages/api/payment/*.ts)
      RAZORPAY_KEY_ID?: string;
      RAZORPAY_KEY_SECRET?: string;
      // Twilio signature verification (src/pages/api/voice/*.ts)
      TWILIO_AUTH_TOKEN?: string;
      // Manual Google Indexing API trigger (src/pages/admin/index-ping.astro)
      AUTO_INDEX_TOKEN?: string;
      // Driver GPS ping auth (src/pages/api/gps/update.ts)
      GPS_ADMIN_TOKEN?: string;
    }
  }
}
