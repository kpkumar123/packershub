...
//   WhatsApp flow is completely unaffected. To turn the backups on:
//   wrangler kv namespace create TRACKING_KV (then add the binding in
//   wrangler.toml) for the Tracking ID, and/or wrangler secret put
//   RESEND_API_KEY + wrangler secret put LEAD_NOTIFY_EMAIL for the email
//   backup — see CHANGELOG.md → "v10.3 — AI Features Release" for the full
//   secrets list (SMS/WhatsApp follow-up channels are optional add-ons).
...
export async function POST({ request }: { request: Request }) {
  try {
    ...
    // 3) Trigger instant follow-up sequence (confirmation email + team SMS).
    //    Fire-and-forget — never awaited so it cannot delay the response.
    //    The sequence API handles h4/h24/h72 steps via Cloudflare Cron Triggers.
    const baseUrl = 'https://www.packershub.in'; // update if domain changes
    fetch(`${baseUrl}/api/follow-up`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        trackingId,
        ...
        fromCity: record.fromCity || undefined,
        toCity: record.toCity || undefined,
        moveDate: record.moveDate || undefined,
        moveType: record.moveType || undefined,
        estimate: record.estimate ?? undefined,
        sequenceStep: 'instant',
      }),
