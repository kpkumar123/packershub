...
## v10.7.35 — Build-breaking JSX comment syntax fix (actual root cause of the build failure)

- **Root cause of the `astro build` failure found and fixed.** Raw HTML
  comments (`<!-- text -->`) had been used *inside* JSX-style `{}` template
  expressions (e.g. `{condition && ( <!-- comment --> <div>...` or
  `{items.map(x => ( <!-- comment --> <div>...`). Astro's compiler parses
  the contents of `{}` expressions as JavaScript/JSX, where `<!-- -->` is
  not a valid comment — only `{/* ... */}` is. This produced the exact
  `[CompilerError] Unexpected token` at `[state]/[city].astro:157:13` from
  the build log, and would have failed again at the next occurrence had
  only that one line been patched.
- Found and fixed **13 occurrences** across 3 files by converting them to
  proper JS/JSX comments:
  - `src/pages/index.astro` — 4 occurrences (inside the homepage routes
    `.map()` block: "Accent bar", "From", "Dotted connector", "To").
  - `src/pages/[state]/[city].astro` — 5 occurrences (the "Page last
    updated" freshness block, and the "Section Header" / "Tab Navigation" /
    "Within State Panel" / "Interstate Panel" comments inside the Popular
    Routes IIFE `{(() => { ... return (...) })()}`).
  - `src/pages/[state]/index.astro` — 5 occurrences ("Section Header",
    "Routes grid", "Icon column", "Route text", "Arrow" inside the state
    page's Popular Routes `.map()` block).
  - Comments that sit in plain Astro template markup (outside any `{}`
    expression) were left as `<!-- -->` since those are valid there and
    render correctly.
- **Second, non-fatal issue also fixed:** `RazorpayCheckout.astro`'s
  frontmatter JSDoc comment contained the literal text `<script>` inside a
  sentence ("...it contributes only a `<script>`, no DOM nodes."). Vite's
  dependency pre-scanner does a lightweight text scan for `<script>` tags
  in `.astro` files and isn't comment-aware, so it misread this as a real
  script tag opening and tried to bundle everything after it as JS —
  matching the `[PARSE_ERROR] ... 1 │ , no DOM nodes.` warning in the build
  log. This scan failure was non-fatal (Vite fell back and continued), but
  is fixed by rewording the comment to "a script tag" instead.
- These bugs were missed by the v10.7.33/v10.7.34 audits because those
  checked for encoding corruption and TypeScript/Node syntax validity —
  neither catches invalid comment syntax inside Astro's JSX expression
  regions, which only the Astro compiler itself flags.
- Verified: brace/paren balance confirmed across all 4 edited files;
  systematic project-wide scan confirms zero remaining `<!-- -->` comments
  inside any `{}` expression in any `.astro` file.
